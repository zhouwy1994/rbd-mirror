...
if [ $? -eq 0 ] ;then
	...
	my_exit 2 "$fail_msg" "local: get rbd mirror $image_name status false"
fi
...
then 
		...
		if [ $? -eq 0 ] ; then 
			...
			my_exit 3  "$fail_msg" "local: image#$image_name disable is false"
		fi
else
		add_log "ERROR" "remote: rbd mirror process is not exist"
		my_exit 4 "$fail_msg" "remote: rbd mirror process is not exist"
fi
